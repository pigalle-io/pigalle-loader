...
const {PluginNotFoundError} = require('./import-error')
...
class Importer extends PigalleBaseClass {
  ...
  constructor (name, library) {
    super(name, library)
    if (!name) {
      throw new UndefinedError(`Module name is missing`)
    }
    if (typeof name !== 'string') {
      throw new InvalidTypeError(`Module name must be a string (name=${name},typeof=${typeof name}`)
    }
    this.name = name
  ...
  import () {
    if (!this.library) {
      this.library = this.require(this.doesPluginPathExists())
    }
    return this
  }
...
function importer (name) {
  return Importer.factory({name: name})
}
